...
- `totaal`: aantal medewerkers (HR-lijst), aantal deelnemers (medewerkers die in het e-learningportaal Power UP geregistreerd zijn), aantal combinaties medewerker × training en de verdeling over de statussen `afgerond`, `bezig` en `niet_gestart` (aantallen en percentages), plus de gemiddelde voortgang van wie bezig is. Medewerkers die nog niet zijn ingelogd in Power UP tellen als `niet_gestart`;
...
1. **Voortgang** — één of twee zinnen over de totale stand (vooral deelname, % afgerond en de stand van de verplichte trainingen).
