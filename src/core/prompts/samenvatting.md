...
- `totaal`: aantal medewerkers, aantal combinaties medewerker × training en de verdeling over de statussen `afgerond`, `bezig`, `niet_gestart` en `niet_ingeschreven` (aantallen en percentages), plus de gemiddelde voortgang van wie bezig is;
- `perTraining`, `perBedrijf`, `perAfdeling`: dezelfde verdeling per groep;
...
1. **Voortgang** — één of twee zinnen over de totale stand (vooral % afgerond en % niet ingeschreven).
